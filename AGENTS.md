...
7. DeviantArt media is normalized into one model (`photo`, `video`, `animation`, `document`) before Telegram. A video DTO `media.baseUri` is only a poster; playable URLs must come from video sources (`media.types[].t === "video"` or OAuth `videos[]`). Literature works are inline `document` payloads, never fake images.
