...
- 来源入口**每个作品只有一个、绝不重复**：一律在媒体发出后补发一行小号蓝色 `source` 文本超链接回到原作品页（JSON `text_link` 各路径都可靠，不展开链接预览）。单图/视频的图片下方另有「📲 Daviewer 客户端」内联按钮，指向 [DAViewer 客户端下载页](https://redtidev1918.github.io/daviewer/#/download)（按钮在 URL 直传 / file_id 重放 / multipart 上传各路径都可靠）；相册（sendMediaGroup 会静默丢弃按钮）只发 `source` 文本行，不补客户端按钮。
